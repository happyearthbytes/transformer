...
IDS = [13600, 19699, 19846, 27478, 27479, 27480, 30073, 30685, 30935, 30976, 31156, 31329, 31447, 31641, 31793, 31855, 31950, 32063, 32097, 32182, 32294, 32423, 32607, 32689, 32758, 32783, 32860, 32940, 32975, 33052, 33127, 33189, 33239, 33295, 33365, 33427, 33550, 33614, 33698, 33750, 34018, 34047, 34082, 34116, 34162, 34209, 34312, 34405, 34533, 34612, 34702, 34751, 34878, 34992, 35092, 35169, 35236, 35306, 35398, 35473, 35561, 35606, 35747, 35925, 36104, 36226, 36452, 36735, 37064, 37160, 37282, 37461, 37523, 37610, 37736, 37806, 37880, 37984, 38143, 38202, 38304, 38401, 38454, 38539, 38622, 38709, 38799, 38892, 38964, 39029, 39127, 39232, 39353, 39435, 39521, 39632, 39700, 39775, 39908, 40009, 40096, 40156, 40370, 40538, 40641, 40769, 40863, 40956, 41055, 41156, 41264, 41343, 41472, 41567, 41685, 41773, 41902, 42048, 42173, 42342, 42473, 42552, 42638, 42736, 42854, 43060, 43254, 43427]

@dataclass
class DownloadConfig:
    url_fmt: str = "https://www.gutenberg.org/cache/epub/{id}/pg{id}.txt"
    ids: list[int] = field(default_factory=lambda :IDS)
    download_path: Path = Path("data/raw_text")

    def get_items(self) -> Generator[tuple[str,str], None, None]:
        for id in self.ids:
            yield (f"{id}.txt", self.url_fmt.format(id=id))
...
class Config:
    ...
    last_time: float = 0.0
    download_txt_path: Path = Path("data/raw_text")
    formatted_txt_path: Path = Path("data/raw_text")
    tokenized_txt_path: Path = Path("data/tokenized_text")
...
class DataHandler:
    class Downloader:
        def __init__(self, download_config: DownloadConfig):
            ...
            self.download_config.download_path.mkdir(parents=True, exist_ok=True)
        
        def download(self):
            for fname, url in self.download_config.get_items():
                download_text = self.download_url(url, fname)
                if download_text is None:
                    continue
                self.write_file(fname=fname, contents=download_text)

        def write_file(self,fname:str,contents:str):
            (self.download_config.download_path / fname).write_text(contents, encoding="utf-8")
        
        def file_found(self, fname:str):
            return (self.download_config.download_path / fname).is_file()

        def download_url(self, url: str, fname: str) -> str | None:
            if self.file_found(fname=fname):
    ...
    class Tokenizer:
        ...
        def save_tokenized(self):
            self.tokenize_dir(src_dir=self.config.formatted_txt_path,dest_dir=self.config.tokenized_txt_path)

        def tokenize_dir(self, src_dir: Path, dest_dir: Path):
            src_files = src_dir.glob('*')
            dest_files = dest_dir.glob('*')
            ...
            for src_file in src_files:
                ...
                if not dest_file.exists():
                    self.tokenize_text(src_file, dest_file)

        def tokenize_text(self, src_file: Path, dest_file: Path):
            print(src_file)
            text = src_file.read_text(encoding="utf-8")
            text = text[:1000]
            doc = self.nlp(text)
            doc.to_disk(path=dest_file)
